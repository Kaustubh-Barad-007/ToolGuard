...
// packages/core/dist/normalization/normalizer.js
function normalizeValue(val, keyName) {
...
function normalizeToolDefinition(raw) {
  const permissions = Array.isArray(raw.permissions) ? [...new Set(raw.permissions.map((p) => String(p).trim().toLowerCase()))].sort() : [];
  const authentication = {
    required: Boolean(raw.authentication?.required),
    type: raw.authentication?.type ? String(raw.authentication.type).trim().toLowerCase() : void 0,
    scopes: Array.isArray(raw.authentication?.scopes) ? [...new Set(raw.authentication.scopes.map((s) => String(s).trim()))].sort() : void 0
  };
  const execution = {
    enabled: Boolean(raw.execution?.enabled),
    command: raw.execution?.command ? String(raw.execution.command).trim().replace(/\\/g, "/") : void 0,
    isolated: raw.execution?.isolated !== void 0 ? Boolean(raw.execution.isolated) : void 0,
    shell: raw.execution?.shell !== void 0 ? Boolean(raw.execution.shell) : void 0,
    timeoutMs: typeof raw.execution?.timeoutMs === "number" ? raw.execution.timeoutMs : void 0
  };
  const inputSchema = raw.inputSchema && typeof raw.inputSchema === "object" ? normalizeValue(raw.inputSchema) : {};
  const outputSchema = raw.outputSchema && typeof raw.outputSchema === "object" ? normalizeValue(raw.outputSchema) : {};
  const metadata = raw.metadata && typeof raw.metadata === "object" ? normalizeValue(raw.metadata) : {};
...
// packages/core/dist/fingerprint/hasher.js
var import_fast_json_stable_stringify = __toESM(require_fast_json_stable_stringify(), 1);
function computeFingerprint(normalized) {
  const canonicalJson = (0, import_fast_json_stable_stringify.default)(normalized);
...
// packages/core/dist/diff/differ.js
function deepDiff(beforeObj, afterObj, prefix = "") {
  const changes = [];
  const allKeys = /* @__PURE__ */ new Set([...Object.keys(beforeObj || {}), ...Object.keys(afterObj || {})]);
  for (const key of allKeys) {
    const currentPath = prefix ? `${prefix}.${key}` : key;
    const beforeVal = beforeObj ? beforeObj[key] : void 0;
    const afterVal = afterObj ? afterObj[key] : void 0;
    if (beforeVal === void 0 && afterVal !== void 0) {
      ...
      const nested = deepDiff(beforeVal, afterVal, currentPath);
      changes.push(...nested);
...
function diffToolDefinitions(baseline, current) {
  return deepDiff(baseline, current);
}

// packages/core/dist/risk/rules.js
var RISK_RULES = [
  // 1. Command execution enabled
  {
    ...
    applies: (ch) => {
      return ch.path === "execution.enabled" && ch.after === true && ch.before !== true || ch.path.startsWith("permissions") && Array.isArray(ch.after) && ch.after.includes("execute") && (!Array.isArray(ch.before) || !ch.before.includes("execute"));
    },
    evaluate: () => ({
      severity: "high",
...
function evaluateChangeRisk(change) {
...
function determineOverallSeverity(changes) {
...
// packages/core/dist/explanation/explainer.js
function formatDriftExplanation(toolName, changes) {
  if (changes.length === 0) {
...
`);
  for (const change of changes) {
    const icon = change.severity === "high" ? "\u{1F534} HIGH RISK" : change.severity === "medium" ? "\u{1F7E1} REVIEW" : "\u{1F535} LOW";
...
// packages/core/dist/security/redaction.js
...
// packages/core/dist/baseline/manager.js
var BaselineManager = class {
  /**
   * Creates a new baseline record from a list of discovered tools.
   */
  static createBaseline(tools, projectId = "local-project", createdBy = "developer", version = 1) {
   ...
   */
  static compare(tools, baseline) {
    const scanId = `scan-${Date.now()}`;
    const scannedAt = (/* @__PURE__ */ new Date()).toISOString();
    const toolStatuses = [];
    let driftCount = 0;
    let highestSeverity = "none";
    const currentToolsMap = /* @__PURE__ */ new Map();
    ...
    for (const [toolId, baselineEntry] of Object.entries(baseline.tools)) {
      ...
      if (currentFingerprint.hash === baselineEntry.fingerprint) {
        ...
        const trustStatus = severity === "high" ? "HIGH RISK" : "REVIEW";
        if (severity === "high") {
   ...
   */
  static acceptToolChange(existingBaseline, tool, actorId = "developer") {
...
// packages/core/dist/discovery/json-adapter.js
var import_fs = require("fs");
var path = __toESM(require("path"), 1);
var GenericJsonAdapter = class {
  name = "Generic JSON Tool Adapter";
  async detect(workspacePath) {
    ...
    for (const file of singleFiles) {
      try {
        const stat = await import_fs.promises.stat(file);
        if (stat.isFile())
          return true;
      } catch {
      }
  ...
  async discover(workspacePath) {
    ...
    for (const dir of candidateDirs) {
      try {
        ...
        for (const file of files) {
          ...
          try {
            const content = await import_fs.promises.readFile(fullPath, "utf8");
    ...
    for (const file of singleFiles) {
      try {
        const content = await import_fs.promises.readFile(file, "utf8");
        const parsed = JSON.parse(content);
        if (Array.isArray(parsed)) {
          for (const item of parsed) {
            if (item && typeof item === "object" && item.name) {
...
// packages/core/dist/discovery/mcp-adapter.js
var import_fs2 = require("fs");
var path2 = __toESM(require("path"), 1);
var CANDIDATE_MCP_FILES = (workspacePath) => [
  path2.join(workspacePath, ".cursor", "mcp.json"),
  path2.join(workspacePath, "mcp.json"),
  path2.join(workspacePath, ".toolguard", "mcp.json"),
  path2.join(workspacePath, ".vscode", "mcp.json"),
  path2.join(workspacePath, ".windsurf", "mcp_config.json"),
  path2.join(workspacePath, ".codeium", "windsurf", "mcp_config.json"),
  path2.join(workspacePath, "cline_mcp_settings.json"),
  path2.join(workspacePath, ".cline", "mcp_settings.json"),
  path2.join(workspacePath, ".roo", "mcp_settings.json")
];
var McpToolAdapter = class {
  name = "Model Context Protocol (MCP) Adapter";
  async detect(workspacePath) {
    for (const file of CANDIDATE_MCP_FILES(workspacePath)) {
      try {
        const stat = await import_fs2.promises.stat(file);
        if (stat.isFile())
          return true;
      } catch {
      }
  ...
  async discover(workspacePath) {
    ...
    for (const file of CANDIDATE_MCP_FILES(workspacePath)) {
      try {
        const content = await import_fs2.promises.readFile(file, "utf8");
        const parsed = JSON.parse(content);
        ...
        for (const [serverName, serverConfig] of Object.entries(servers)) {
          ...
          tools.push({
            ...
            metadata: {
              source: "mcp",
              file: path2.relative(workspacePath, file)
            }
          });
        }
      } catch {
...
// packages/core/dist/discovery/package-json-adapter.js
var import_fs3 = require("fs");
var path3 = __toESM(require("path"), 1);
var PackageJsonAdapter = class {
  name = "Package.json Script & Tool Adapter";
  async detect(workspacePath) {
    const pkgPath = path3.join(workspacePath, "package.json");
    try {
      const stat = await import_fs3.promises.stat(pkgPath);
      return stat.isFile();
    } catch {
      return false;
    }
  ...
  async discover(workspacePath) {
    ...
    const tools = [];
    try {
      const raw = await import_fs3.promises.readFile(pkgPath, "utf8");
      ...
      for (const [scriptName, scriptCmd] of Object.entries(scripts)) {
        ...
        if (cmdLower.includes("rm ") || cmdLower.includes("del ") || cmdLower.includes("rimraf") || cmdLower.includes("mkdir") || cmdLower.includes("cp ") || cmdLower.includes("mv ") || cmdLower.includes("clean") || cmdLower.includes("build") || cmdLower.includes("dist")) {
          permissions.push("write");
        }
        if (cmdLower.includes("curl ") || cmdLower.includes("wget ") || cmdLower.includes("fetch") || cmdLower.includes("http://") || cmdLower.includes("https://") || cmdLower.includes("publish") || cmdLower.includes("deploy") || cmdLower.includes("git push")) {
...
// packages/core/dist/discovery/python-adapter.js
var import_fs4 = require("fs");
var path4 = __toESM(require("path"), 1);
var PythonProjectAdapter = class {
  name = "Python Project & Agent Adapter";
  async detect(workspacePath) {
    const candidateFiles = [
      path4.join(workspacePath, "pyproject.toml"),
      path4.join(workspacePath, "requirements.txt"),
      path4.join(workspacePath, "Pipfile"),
      path4.join(workspacePath, "setup.py")
    ];
  ...
  async discover(workspacePath) {
    ...
    const pyprojectPath = path4.join(workspacePath, "pyproject.toml");
    try {
      const content = await import_fs4.promises.readFile(pyprojectPath, "utf8");
    ...
    const reqPath = path4.join(workspacePath, "requirements.txt");
    try {
      const content = await import_fs4.promises.readFile(reqPath, "utf8");
      const lines = content.split(/\r?\n/);
      const knownTools = [
...
// packages/core/dist/discovery/makefile-adapter.js
var import_fs5 = require("fs");
var path5 = __toESM(require("path"), 1);
var MakefileAdapter = class {
  name = "Makefile Target Adapter";
  async detect(workspacePath) {
    const candidates = ["Makefile", "makefile", "GNUmakefile"];
    for (const name of candidates) {
      try {
        const stat = await import_fs5.promises.stat(path5.join(workspacePath, name));
        if (stat.isFile())
          return true;
      } catch {
      }
  ...
  async discover(workspacePath) {
    ...
    for (const name of candidates) {
      const fullPath = path5.join(workspacePath, name);
      try {
    ...
    const tools = [];
    try {
      const content = await import_fs5.promises.readFile(makefilePath, "utf8");
      const lines = content.split(/\r?\n/);
      ...
      const flushTarget = () => {
        ...
        const permissions = ["read", "execute"];
        if (cmdLower.includes("rm ") || cmdLower.includes("del ") || cmdLower.includes("mkdir") || cmdLower.includes("cp ") || cmdLower.includes("mv ") || cmdLower.includes("build") || cmdLower.includes("clean") || cmdLower.includes("sed ")) {
          permissions.push("write");
        }
        if (cmdLower.includes("curl ") || cmdLower.includes("wget ") || cmdLower.includes("http://") || cmdLower.includes("https://") || cmdLower.includes("docker push") || cmdLower.includes("git push") || cmdLower.includes("ssh ")) {
          permissions.push("network");
        }
...
// packages/core/dist/discovery/vscode-tasks-adapter.js
var import_fs6 = require("fs");
var path6 = __toESM(require("path"), 1);
var VSCodeTasksAdapter = class {
  name = "VS Code Workspace Tasks Adapter";
  async detect(workspacePath) {
    const tasksFile = path6.join(workspacePath, ".vscode", "tasks.json");
    try {
      const stat = await import_fs6.promises.stat(tasksFile);
      return stat.isFile();
    } catch {
      return false;
    }
  ...
  async discover(workspacePath) {
    ...
    const tools = [];
    try {
      const content = await import_fs6.promises.readFile(tasksFile, "utf8");
      const cleanJson = content.replace(/\/\/.*$/gm, "").replace(/\/\*[\s\S]*?\*\//g, "");
      const parsed = JSON.parse(cleanJson);
      const tasks = Array.isArray(parsed.tasks) ? parsed.tasks : [];
      for (const t of tasks) {
        if (!t.label && !t.taskName)
          continue;
        ...
        const permissions = ["read", "execute"];
        if (cmdLower.includes("rm ") || cmdLower.includes("del ") || cmdLower.includes("clean") || cmdLower.includes("mkdir") || cmdLower.includes("build")) {
          permissions.push("write");
        }
        if (cmdLower.includes("http://") || cmdLower.includes("https://") || cmdLower.includes("curl") || cmdLower.includes("wget") || cmdLower.includes("publish") || cmdLower.includes("deploy")) {
          permissions.push("network");
        }
        if (cmdLower.includes("sudo ") || cmdLower.includes("admin")) {
          permissions.push("admin");
...
// packages/core/dist/discovery/workflow-adapter.js
var import_fs7 = require("fs");
var path7 = __toESM(require("path"), 1);
var WorkflowAdapter = class {
  name = "GitHub Actions Workflow Tool Adapter";
  async detect(workspacePath) {
    const workflowsDir = path7.join(workspacePath, ".github", "workflows");
    try {
      const stat = await import_fs7.promises.stat(workflowsDir);
      if (stat.isDirectory()) {
        const files = await import_fs7.promises.readdir(workflowsDir);
    ...
    try {
      ...
      for (const file of files) {
        if (!file.endsWith(".yml") && !file.endsWith(".yaml"))
          continue;
        const filePath = path7.join(workflowsDir, file);
        try {
          const raw = await import_fs7.promises.readFile(filePath, "utf8");
...
// packages/core/dist/discovery/universal.js
...
async function discoverWorkspaceTools(workspacePath) {
...
// packages/core/dist/storage/file-storage.js
...
var ToolAuthenticationSchema = external_exports.object({
  required: external_exports.boolean().default(false),
  type: external_exports.string().optional(),
  scopes: external_exports.array(external_exports.string()).optional()
});
var ToolExecutionSchema = external_exports.object({
  enabled: external_exports.boolean().default(false),
  command: external_exports.string().optional(),
  isolated: external_exports.boolean().optional(),
  shell: external_exports.boolean().optional(),
  timeoutMs: external_exports.number().optional()
});
var ToolDefinitionSchema = external_exports.object({
  id: external_exports.string().optional(),
  name: external_exports.string().min(1, "Tool name is required"),
...
var DriftChangeSchema = external_exports.object({
  path: external_exports.string(),
  type: external_exports.enum(["added", "removed", "changed"]),
  ...
  whyItMatters: external_exports.string()
});
var BaselineToolEntrySchema = external_exports.object({
  toolId: external_exports.string(),
  name: external_exports.string(),
  normalizedDefinition: external_exports.record(external_exports.unknown()),
  fingerprint: external_exports.string().length(64),
  metadata: external_exports.record(external_exports.unknown()).optional(),
  createdAt: external_exports.string()
});
var BaselineSchema = external_exports.object({
  baselineId: external_exports.string(),
  projectId: external_exports.string(),
  version: external_exports.number().int().positive(),
  createdAt: external_exports.string(),
  createdBy: external_exports.string(),
  algorithm: external_exports.literal("SHA-256"),
  tools: external_exports.record(BaselineToolEntrySchema),
  toolCount: external_exports.number().int().nonnegative()
});
var AcceptDriftRequestSchema = external_exports.object({
  projectId: external_exports.string(),
  eventId: external_exports.string(),
  toolId: external_exports.string(),
  actorId: external_exports.string(),
  actorEmail: external_exports.string().optional(),
...
// packages/core/dist/storage/file-storage.js
var FileBaselineStorage = class {
  /**
   * Reads the baseline file from the local workspace.
   */
  static async loadLocalBaseline(workspacePath) {
    const target = path8.join(workspacePath, DEFAULT_PATHS.BASELINE_FILE);
    try {
      const content = await import_fs8.promises.readFile(target, "utf8");
   ...
   */
  static async saveLocalBaseline(workspacePath, baseline) {
    const dir = path8.join(workspacePath, ".toolguard");
    await import_fs8.promises.mkdir(dir, { recursive: true });
    const target = path8.join(dir, "baseline.json");
    await import_fs8.promises.writeFile(target, JSON.stringify(baseline, null, 2), "utf8");
  }
};

// packages/cli/src/index.ts
var program2 = new Command();
program2.name("toolguard").description("ToolGuard \u2014 You trusted the tool. Did the tool stay the same?").version("1.0.0");
program2.command("init").description("Initialize ToolGuard in the current workspace and create baseline").option("-y, --yes", "Automatically confirm baseline creation", false).action(async (options) => {
...
`);
    if (tools.length === 0) {
      ...
      const toolsDir = path9.join(cwd, ".toolguard", "tools");
      await import_fs9.promises.mkdir(toolsDir, { recursive: true });
      const starterTool = {
        id: "dev-runner",
        name: "dev-runner",
      ...
      await import_fs9.promises.writeFile(
        path9.join(toolsDir, "dev-tools.json"),
...
`);
    ...
    const baseline = BaselineManager.createBaseline(tools, path9.basename(cwd));
...
program2.command("scan").description("Scan current tools against the trusted baseline").option("--ci", "Run in CI mode with exit code enforcement", false).option("--fail-on <severity>", "Fail CI if severity matches or exceeds (low|medium|high)", "high").option("--json", "Output results as JSON", false).action(async (options) => {
  const cwd = process.cwd();
  try {
...
`);
    ...
    if (options.ci) {
      ...
      const minThreshold = severityOrder[options.failOn.toLowerCase()] || 3;
      const highestDetected = scanResult.highestSeverity !== "none" ? severityOrder[scanResult.highestSeverity] : 0;
...
program2.command("status").description("Show current ToolGuard protection status").action(async () => {
  const cwd = process.cwd();
  const baseline = await FileBaselineStorage.loadLocalBaseline(cwd);
...
program2.command("baseline").description("View or update trusted baseline").option("-c, --create", "Force recreate baseline with current tools", false).action(async (options) => {
  const cwd = process.cwd();
  if (options.create) {
    const tools = await discoverWorkspaceTools(cwd);
    const baseline2 = BaselineManager.createBaseline(tools, path9.basename(cwd));
    await FileBaselineStorage.saveLocalBaseline(cwd, baseline2);
...
program2.command("explain <toolName>").description("Explain trust drift and security implications for a specific tool").action(async (toolName) => {
  const cwd = process.cwd();
  const baseline = await FileBaselineStorage.loadLocalBaseline(cwd);
  if (!baseline) {
...
program2.command("export").description("Export the trusted baseline for import into Web Dashboard or CI").option("-o, --output <file>", "Save output to a specific JSON file").action(async (options) => {
  const cwd = process.cwd();
  const baseline = await FileBaselineStorage.loadLocalBaseline(cwd);
  if (!baseline) {
  ...
  if (options.output) {
    await import_fs9.promises.writeFile(path9.resolve(cwd, options.output), json, "utf8");
    console.log(`\u2713 Exported baseline to ${options.output}`);
  } else {
    console.log(json);
  }
});
program2.command("sync").description("Check workspace sync and display dashboard link").action(async () => {
  const cwd = process.cwd();
  const baseline = await FileBaselineStorage.loadLocalBaseline(cwd);
  const url = process.env.TOOLGUARD_DASHBOARD_URL || "https://toolguard-app.vercel.app";
  console.log("\n\u{1F6E1} ToolGuard Workspace Sync");
  console.log("\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500");
  if (!baseline) {
  ...
  const tools = await discoverWorkspaceTools(cwd);
  const scanResult = BaselineManager.compare(tools, baseline);
  console.log(`Project:         ${baseline.projectId || path9.basename(cwd)}`);
...
program2.command("dashboard").description("Open the ToolGuard Web Dashboard in browser").action(() => {
  const url = process.env.TOOLGUARD_DASHBOARD_URL || "https://toolguard-app.vercel.app";
...
program2.command("disconnect").alias("reset").description("Disconnect ToolGuard from the current workspace and delete protection baselines").action(async () => {
  const cwd = process.cwd();
  const toolguardDir = path9.join(cwd, ".toolguard");
  console.log("\n\u{1F6E1} ToolGuard Disconnect\n\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500");
  try {
    const exists = await import_fs9.promises.stat(toolguardDir).catch(() => null);
    if (!exists) {
      console.log("No .toolguard configuration found in this workspace.");
      console.log("Workspace is already disconnected.\n");
      return;
    }
    await import_fs9.promises.rm(toolguardDir, { recursive: true, force: true });
